...
installer {
    ...
    LinuxBuild {
        ...
        QMAKE_POST_LINK += && tar --exclude='release/package' -cjf release/package/QGroundControl.tar.bz2 release --transform 's/release/qgroundcontrol/'
    }
    AndroidBuild {
        #-- TODO: This uses hardcoded paths. It should use $${DESTDIR}
        QMAKE_POST_LINK += && mkdir -p $${DESTDIR}/package
        QMAKE_POST_LINK += && make install INSTALL_ROOT=$${DESTDIR}/android-build/
        QMAKE_POST_LINK += && androiddeployqt --input android-libQGroundControl.so-deployment-settings.json --output $${DESTDIR}/android-build --deployment bundled --gradle --sign $${BASEDIR}/android/coex_qgc.jks qgc_coex_key --storepass $$(ANDROID_STOREPASS) --keypass $$(ANDROID_KEYPASS)
